...
Point getdestn_obj(int id)
{
	Point to_return;
	if(id==0)  to_return = Point(3,3);
	else to_return = Point(3,4);
	return to_return;
}
...
void deliver(char *data) // code for delivery bot
{
	...
	else // reached destination
	{
		...
		lastsigd[bot_number] = to_send[2];
		sendString(to_send);
		delivery[bot_number].is_free = true;
	}
}
...
void trigger_dbot(int bot_number)
{
	...
	to_send[3] = '\0';
	lastsigd[bot_number] = to_send[2];
	delivery[bot_number].destn = getdestn_obj(bot_number);
	sendString(to_send);
}
